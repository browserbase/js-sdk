...
import { asLocators } from './locator-generators'
import type { Language, Quote } from './locator-generators'
...
function parseLocator(
  ...
  const preferredQuote = params
    .map((p) => p.quote)
    .filter((quote) => '\'"`'.includes(quote))[0] as Quote | undefined
  return {
      ...
      t = t
        .replace(/(?:r)\$(\d+)(i)?/g, (_, ordinal, suffix) => {
          ...
          if (
            ...
            return (
              escapeForAttributeSelector(new RegExp(param.text), false) +
              (suffix || '')
            )
